...
const EditPerro = (props) => {
  ...
  const [claseCard, setClaseCard] = useState("");

  const loaderCard = (loader) => {
    if (loader) {
      setClaseCard("whirl double-up");
    } else {
      setClaseCard("");
    }
  };
  ...
  useEffect(() => {
    ...
    const getPerro = () => {
      const url = `/perro/details/${idPerro}`;
      getApi(url, "GET", null, (status, data, message) => {
        console.log(status, data, message);
        if (!status) {
          console.log(message);
          return props.history.push(`/mantenedores/perro`);
        }

        setNombre(data.perro_nombre);
        setColor(data.perro_color);
        setEdad(data.perro_edad);
        setRaza({
          value: data.raza.id,
          label: data.raza.raza_nombre,
          raza: data.raza,
        });
        setClaseCard(false);
      });
  ...
  const onSubmit = (e) => {
    ...
    const url = `/perro/update/${idPerro}`;
    getApi(url, "PUT", datos, (status, data, message, re) => {
      console.log(status, data, message, re);
      if (status) {
        swal({
          title: "Correcto",
          text: "Perro Actualizado con exito.",
          icon: "success",
          buttons: true,
        }).then((willDelete) => {
          if (!willDelete) {
    ...
    <ContentWrapper>
      ...
      <div className="p-0">
        <Row className="d-flex justify-content-center mb-1">
          <Col xl="12">
            <div className={`card card-default ${claseCard}`}>
              <div className="card-header mb-0">
                <div className="text-tittle-card mt-1">
                  <em className="fas fa-sync"></em>&nbsp;Actualizar Perro.
                </div>
                <hr></hr>
              </div>
